        ...
        <div className="tab-box-v">
          <div className="all__search-bar">
            ...
            <form>
              ...
              <select
								className="all__input"
								onInput={event => this.setState({ contributors: event.target.value })}
							>
              ...
              <select
								className="all__input"
								onInput={event => this.setState({ category: event.target.value })}
							>
          ...
          <div className="results">
            {allPools.map(pool => {
						...
						console.log([this.state.amount, this.state.contributors, this.state.category, this.state.rate, this.state.keyword])
							if (this.state.amount && pool.amount > this.state.amount) {
								return
							}
							if (this.state.contributors && pool.numOfContributors > this.state.contributors) {
								return
							}
							if (this.state.category && pool.category !== this.state.category) {
								return
							}
              ...
              return <div key={pool._id} className="all__card">
                  <div className="all__thumbnail" onClick={() => this.handleClick(pool._id)} style={{ backgroundImage: "url(https://tribwxmi.files.wordpress.com/2013/05/mustache-web.jpeg)" }} alt="" />
